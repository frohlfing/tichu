...
class Agent(Player):
    ...
    :ivar ai_level: Das Level der KI (aktuell nur zur Info).
    """
    ...
    def __init__(self, player_name: Optional[str] = None, ai_level: int = 1, player_id: Optional[str] = None):
        ...
        logger.debug(f"Agent '{self._player_name}' (ID: {self._player_id}, Level: {self.ai_level}) erstellt.")
    ...
    async def notify(self, message_type: str, data: dict):
        ...
        logger.debug(f"Agent {self._player_name} hat Nachricht empfangen: Typ='{message_type}', Daten='{data}'")
