...
from tichu.cards import CARD_DOG, CARD_MAH, CARD_DRA, CARD_PHO, is_wish_in, parse_cards
...
# Label einer Kombination anzeigen
def print_figure(figure: tuple):  # pragma: no cover
    print(stringify_figure(figure))


# Typ, Länge und Wert der Kartenkombination ermitteln
# cards: Karten der Kombination, z.B. [(8,4),(8,2),(8,1)] - MUTABLE
...
def build_combinations(hand: List[tuple]) -> List[tuple]:  # todo Parameter und Rückgabe als Tuple
...
# return: [(Karten, (Typ, Länge, Wert)), ...]
def remove_combinations(combis: List[tuple], cards: List[tuple]):  # todo Parameter und Rückgabe als Tuple
...
def build_action_space(combis: Tuple[tuple], trick_figure: tuple, unfulfilled_wish: int) -> List[tuple]:  # todo Rückgabe als Tuple
...
def calc_statistic(player: int, hand: Tuple[tuple], combis: Tuple[tuple], number_of_cards: Tuple[int], trick_figure: tuple, unplayed_cards: Tuple[tuple]) -> dict:
...
if __name__ == "__main__":
    result = build_combinations(parse_cards('Ph R5 S4 B4 G4 R4 S3 B3 G3 R3 S2 B2 G2 R2'))  # Superblatt
    print(len(result))
